...
export const usePurchaseStore = defineStore('purchase', () => {
  ...
  const postPurchase = async (purchase) => {
    ...
    return await Store(purchase).then(response => {
      ...
      localStorage.setItem('purchases', JSON.stringify(purchases.value))
      message(`Compra ${response.id} registrada con éxito`, "positive")
      return true;
    }).catch(error => {
      message(error, "negative")
      return false
  ...
  const putPurchase = async (id, purchase) => {
    ...
    return await Update(id, purchase).then(response => {
      ...
      localStorage.setItem('products', JSON.stringify(purchases.value))
      message(`Compra ${response.id} actualizada con éxito`, "positive")
      return true;
    }).catch(error => {
      message(error, "negative")
      return false
  ...
  const deletePurchase = async (id) => {
    ...
    return await Delete(id).then(response => {
      ...
      localStorage.setItem('purchases', JSON.stringify(purchases.value))
      message(`Compra ${response.id} eliminada con éxito`, "positive")
      return true;
    }).catch(error => {
      message(error, "negative")
      return false
